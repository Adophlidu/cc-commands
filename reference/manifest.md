...
| `roles` | string[] | Which agent role files were generated under `.claude/d/agents/`. Subset of the full role list depending on `projectType` (e.g. no `d-frontend` or `d-ui` for a `cli` project). |
...
| `specCounter` | number | Auto-incrementing integer. Incremented each time a new spec is created under `docs/specs/`. Yields zero-padded filenames like `docs/specs/0001-auth-flow.md`. |
